...
"""
Special version of the gpTableCompare utility.

The difference between this and the standard version is that the target tables names are named like the source,
with "_encrypt" appended.


"""
databases = ["coddev  (lneddgt405)",
             "codat   (lneddgt405)",
             "codpt   (lneddgt405)",
             "codpft  (lneddgt405)",
             "codprod (lneddgp211)"]
...
class GpCompareApp(npyscreen.NPSAppManaged):
    ...
    def MainLine(self):
        ...
        tableFile = TestApp.get_tablelist()
        if tableFile != None:
            self.update_progress("Table List file is " + tableFile)
            tableIncludes = getTableList(tableFile)
        else:
        ...
        srcSuffix = None
        tgtSuffix = "encrypted"
        srcTables = getTables (dbName=srcDbName, schemaName=srcSchemaName, server=srcServer, tableIncludes=tableIncludes, suffix=srcSuffix);
        tgtTables = getTables (dbName=tgtDbName, schemaName=tgtSchemaName, server=tgtServer, tableIncludes=tableIncludes, suffix=tgtSuffix);
        ...
        triage_dict = triageLists(srcTables, tgtTables, srcSuffix= srcSuffix, tgtSuffix=tgtSuffix);
        ...
        if len(cmprTables) == 0:
            self.update_progress("No tables to compare")
        else:
            ...
            srcDDL = getTableDDLs (dbName=srcDbName, schemaName=srcSchemaName, server=srcServer, tableList=cmprTables, suffix=srcSuffix);
            self.update_progress("Getting target table definitions")
            tgtDDL = getTableDDLs (dbName=tgtDbName, schemaName=tgtSchemaName, server=tgtServer, tableList=cmprTables, suffix=tgtSuffix);
            ...
            for table in cmprTables:
                ...
                compareResults = compareTables(srcDef=srcDDL[table], tgtDef=tgtDDL[table + '_' + tgtSuffix])
        ...
        if mismatch_table_count == 0:
            ...
            for table in mismatchedTables:
                ...
                details = getMismatchDetails(srcDef=srcDDL[table], tgtDef=tgtDDL[table + '_' + tgtSuffix])
...
def getTables(dbName=None, schemaName=None, tableIncludes=None, server='localhost', suffix=None):
...
# names are converted to lower case to facilitate comparisons between DBMSs
    ...
    if tableIncludes == None:
        ...
        table_select = " SELECT t.relname FROM pg_class t JOIN pg_namespace s ON s.oid = t.relnamespace"           + \
                       ...
                       " AND not exists (select '1' from pg_partition_rule part where part.parchildrelid = t.oid)" + \
                       " AND s.nspname = '" + schemaName + "'"                                                     + \
                       " AND t.relname = '" + tableIncludes[0] + suffix + "'"                                      + \
                       " ORDER BY 1;"
    elif len(tableIncludes) > 1:
...
def triageLists(srcList, tgtList, srcSuffix=None, tgtSuffix=None):
...
#  - tables defined in both the source and target schemas.
    ...
    # strip the suffix from the source table names:
    if srcSuffix == None:
        srcSuffix = ""
    else:
        srcSuffix = '_' + srcSuffix

    srcBaseList = []
    for entry in srcList:
        if entry.find(srcSuffix) == 0:
            srcBaseList.append(entry)
        else:
            srcBaseList.append(entry[:entry.find(srcSuffix)])
        
    # strip the suffix from the target table names:
    if tgtSuffix == None:
        tgtSuffix = ""
    else:
        tgtSuffix = '_' + tgtSuffix

    tgtBaseList = []
    for entry in tgtList:
        if entry.find(tgtSuffix) == 0:
            tgtBaseList.append(entry)
        else:
            tgtBaseList.append(entry[:entry.find(tgtSuffix)])
        
        
    # use the difflib utilities to document differences between the two lists:
    deltas = list(difflib.ndiff(srcBaseList, tgtBaseList))
    ...
    for line in deltas:
        if line.find("_1_prt_") >= 0:  # table partitions:  ignore
            ...
            srcOnlyList.append(line[2:] + srcSuffix)
        elif line[0:1] == '+':         # tables only in the 2nd (target) list
            tgtOnlyList.append(line[2:] + tgtSuffix)
...
def getTableDDLs(dbName=None, schemaName=None, tableList=None, server='localhost', suffix=None):
...
#   - the value is the CREATE TABLE statement, including any indexes.
    ...
    if tableList == None:
        ...
        for table in tableList:
            table = table.strip()
            if table != '':
                dump_cmd.append('--table=' + schemaName + '.' + table + suffix)
